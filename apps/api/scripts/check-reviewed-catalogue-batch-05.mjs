...
const journal = readFileSync(resolve(root, 'compendium/source/manual-review-batch-05.md'), 'utf8');
const lines = [...journal.matchAll(/^\| (\d+) \| `([^`]+)` \|/gm)];
assert.equal(lines.length, 400, 'Journal must contain 400 rows');
for (let i = 0; i < lines.length; i++) {
  assert.equal(Number(lines[i][1]), i + 1601, `Incorrect journal index at row ${i + 1601}`);
  assert.equal(lines[i][2], catalogue.pages[i + 1600].id, `Incorrect article at row ${i + 1601}`);
}
...
const { REVIEWED_CATALOGUE_BATCH_05_LINKS: links, REVIEWED_CATALOGUE_BATCH_05_TAGS: tags } = await import('../dist/compendium-reviewed-catalogue-batch-05.js');
