...
const journal = readFileSync(resolve(root, 'compendium/source/manual-review-batch-06.md'), 'utf8');
const lines = [...journal.matchAll(/^\| (\d+) \| `([^`]+)` \|/gm)];
assert.equal(lines.length, 347, 'Journal must contain 347 rows');
for (let i = 0; i < lines.length; i++) {
  assert.equal(Number(lines[i][1]), i + 2001, `Incorrect journal index at row ${i + 2001}`);
  assert.equal(lines[i][2], catalogue.pages[i + 2000].id, `Incorrect article at row ${i + 2001}`);
}
...
const { REVIEWED_CATALOGUE_BATCH_06_LINKS: links, REVIEWED_CATALOGUE_BATCH_06_TAGS: tags } = await import('../dist/compendium-reviewed-catalogue-batch-06.js');
...
console.log(`347 reviews, ${links.length} contextual links and ${tags.length} privacy tag corrections verified.`);
