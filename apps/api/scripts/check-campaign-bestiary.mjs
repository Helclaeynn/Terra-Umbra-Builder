...
const id='11111111-1111-4111-8111-111111111111';const scenes=[{id:'scene',title:'Rencontre',notes:'',done:false,references:[{creatureId:id,articleId:'campaign-creature:'+id,title:'Créature',category:'Bestiaire de campagne',quantity:2,notes:''}]}];assert.ok(validScenes(scenes));assert.equal(cleanScenes(scenes)[0].references[0].creatureId,id);assert.ok(!validScenes([{...scenes[0],references:[{...scenes[0].references[0],articleId:'campaign-creature:wrong'}]}]));
