...
const canonical=[{...scenes[0],references:[{articleId:'loup-noir',title:'Loup noir',category:'Bestiaire',difficultyId:'standard',quantity:3,notes:''}]}];assert.ok(validScenes(canonical));assert.equal(cleanScenes(canonical)[0].references[0].difficultyId,'standard');assert.ok(!validScenes([{...canonical[0],references:[{...canonical[0].references[0],category:'Règles'}]}]));
