...
const journal = readFileSync(resolve(root, 'compendium/source/manual-review-batch-02.md'), 'utf8');
const lines = [...journal.matchAll(/^\| (\d+) \| `([^`]+)` \|/gm)];
assert.equal(lines.length, 400, 'Journal must contain 400 rows');
for (let i = 0; i < lines.length; i++) {
  assert.equal(Number(lines[i][1]), i + 401, `Incorrect journal index at row ${i + 401}`);
  assert.equal(lines[i][2], catalogue.pages[i + 400].id, `Incorrect article at row ${i + 401}`);
}
...
const { REVIEWED_CATALOGUE_BATCH_02_LINKS: links, REVIEWED_CATALOGUE_BATCH_02_TAGS: tags } = await import('../dist/compendium-reviewed-catalogue-batch-02.js');
