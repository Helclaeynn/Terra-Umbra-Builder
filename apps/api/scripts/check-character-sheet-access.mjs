...
try {
  ...
  await call(other,'POST',`${path}/readers`,{email:gm.email},400);
  await call(player,'POST',`${path}/readers`,{email:other.email},400);
  await call(player,'POST',`${path}/readers`,{email:player.email},400);
  await call(player,'POST',`${path}/readers`,{email:'nobody@example.invalid'},400);
  assert.equal((await call(gm,'GET','/api/characters/shared')).characters.length,0);
  await call(player,'POST',`${path}/readers`,{email:` ${gm.email.toUpperCase()} `});
  await call(player,'POST',`${path}/readers`,{email:gm.email});
  assert.equal((await call(player,'GET',`${path}/readers`)).readers.length,1,'Duplicate grant is idempotent');
  ...
  await call(gm,'POST',`${path}/readers`,{email:stranger.email},400);
