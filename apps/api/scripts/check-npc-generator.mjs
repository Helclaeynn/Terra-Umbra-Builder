...
for(const sex of ['male','female','other','unspecified']){
 ...
 const nationality=c.nationalities.find(x=>x.id===n.nationality);
 const pool=sex==='male'?[...nationality.male,...nationality.neutral]:sex==='female'?[...nationality.female,...nationality.neutral]:sex==='other'?nationality.neutral:[...nationality.male,...nationality.female,...nationality.neutral];
 assert.ok(pool.some(first=>n.name.startsWith(first+' ')));
 assert.ok(nationality.last.some(last=>n.name.endsWith(' '+last)));
}
...
for(const key of ['name','appearance','personality'])assert.ok(new Set(sample.map(n=>n[key])).size>110,key);
const selectedNationality=generateNpcBatch({...request,count:1,nationality:'japonaise',truth:{natureId:'vampire',powerId:'exceptionnel',archetypeId:'predateur'}})[0];
assert.equal(selectedNationality.nationality,'japonaise');assert.equal(selectedNationality.truth.natureId,'vampire');
assert.equal(selectedNationality.truth.powerId,'exceptionnel');assert.ok(selectedNationality.truth.skills.pugilat>selectedNationality.skills.pugilat);
assert.ok(validNpcData(selectedNationality,c));
const guards=Array.from({length:100},(_,i)=>generateNpc('elite','garde','different-guard-'+i));
assert.deepEqual(c.ranges['elite:garde'].attributes.agilite,{min:6,max:8});
assert.deepEqual([...new Set(guards.map(n=>n.attributes.agilite))].sort(),[6,7,8]);
assert.ok(new Set(guards.map(n=>n.talentIds.join('|'))).size>5);
assert.ok(new Set(guards.map(n=>JSON.stringify(n.attributes)+'|'+JSON.stringify(n.skills))).size>90);
assert.equal(generateNpcBatch({...request,truth:{natureId:'x',powerId:'recent',archetypeId:'veilleur'}}),null);
const truthSets=new Set(Array.from({length:80},(_,i)=>generateNpcBatch({...request,count:1,seed:'truth-variation-'+i,truth:{natureId:'garou',powerId:'confirme',archetypeId:'traqueur'}})[0].truth.talentIds.join('|')));
assert.ok(truthSets.size>4,'Truth talents must vary independently of the civilian preset');
for(const power of c.truthPowers){const truthRange=c.truthRanges[`${power.id}:predateur`],set=new Set();
 for(let i=0;i<40;i++){const n=generateNpcBatch({...request,count:1,seed:'power-'+power.id+i,truth:{natureId:'vampire',powerId:power.id,archetypeId:'predateur'}})[0],t=n.truth;
  assert.ok(validNpcData(n,c));assert.ok(t.ptv>=power.minPtv&&(power.maxPtv===null||t.ptv<=power.maxPtv));
  for(const [id,v] of Object.entries(t.attributes))assert.ok(v>=truthRange.attributes[id].min&&v<=truthRange.attributes[id].max);
  for(const [id,v] of Object.entries(t.skills))assert.ok(v>=truthRange.skills[id].min&&v<=truthRange.skills[id].max);
  set.add(JSON.stringify(t.attributes)+'|'+JSON.stringify(t.skills));
 }assert.ok(set.size>30,`Truth attributes and skills must vary at ${power.name}`);
}
const secretTruth=structuredClone(selectedNationality);secretTruth.truth.notes='MJ ONLY KING';secretTruth.truth.abilities='MJ ONLY POWER';
const truthDraft=canonicalNpcDraft(secretTruth);
assert.equal(truthDraft.sections.find(s=>s.id==='profil-verite').audience,'mj');
assert.ok(!JSON.stringify(truthDraft.sections.filter(s=>s.audience!=='mj')).includes('MJ ONLY'));
assert.ok(JSON.stringify(truthDraft.sections.filter(s=>s.audience==='mj')).includes('MJ ONLY KING'));
