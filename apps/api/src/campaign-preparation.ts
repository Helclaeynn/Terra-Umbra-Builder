export type SceneReference={articleId:string;title:string;category:string;quantity:number;notes:string};
...
export function cleanScenes(scenes:CampaignScene[]){return scenes.map(s=>({id:s.id,title:s.title,notes:s.notes,done:s.done,references:s.references.map(r=>({articleId:r.articleId,title:r.title,category:r.category,quantity:r.quantity,notes:r.notes}))}));}
