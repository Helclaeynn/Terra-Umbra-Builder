export type SceneReference={npcId?:string;creatureId?:string;difficultyId?:string;articleId:string;title:string;category:string;quantity:number;notes:string};
...
export function validScenes(value:unknown):value is CampaignScene[]{
 ...
 return value.every(s=>{
  ...
  return s.references.every((r:any)=>{
   ...
   if(r.creatureId!==undefined&&(typeof r.creatureId!=='string'||!/^([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$/i.test(r.creatureId)||r.articleId!=='campaign-creature:'+r.creatureId))return false;
   if(r.difficultyId!==undefined&&(!(r.creatureId||r.category==='Bestiaire')||!['figurant','standard','dangereux','majeur','exceptionnel'].includes(r.difficultyId)))return false;
   if(r.articleId.startsWith('campaign-npc:')&&!r.npcId)return false;
   if(r.articleId.startsWith('campaign-creature:')&&!r.creatureId)return false;
   refs.add(r.articleId);return true;
  });
 });
}
export function cleanScenes(scenes:CampaignScene[]){return scenes.map(s=>({id:s.id,title:s.title,notes:s.notes,done:s.done,references:s.references.map(r=>({articleId:r.articleId,title:r.title,category:r.category,quantity:r.quantity,notes:r.notes,...(r.npcId?{npcId:r.npcId}:{}),...(r.creatureId?{creatureId:r.creatureId}:{}),...(r.difficultyId?{difficultyId:r.difficultyId}:{})}))}));}
