...
export async function registerCampaignSessionRoutes(app:FastifyInstance){
  ...
  app.post<{Params:{id:string};Body:SessionBody}>('/api/campaigns/:id/sessions',async(req,reply)=>{
    ...
    const b=req.body;
    if(!await campaignNpcReferencesAvailable(req.params.id,user.id,b.scenes)||!await campaignBestiaryReferencesAvailable(req.params.id,user.id,b.scenes))return reply.code(400).send({error:'invalid_scene_reference'});
