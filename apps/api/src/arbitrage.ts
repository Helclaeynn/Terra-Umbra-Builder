...
function classify(value: string): TaxonomyKind {
  ...
  if (/(compétence|competence|profil|stat|mécanique|mecanique|règle|regle|armure|talent|don |d[oô]n |puissance|difficulté|difficulte|valeur|tir|pugilat|mêlée|melee|pa propre|points? d’action|points? action|proximité|proximite|jointure|fonction séparée|fonction.{0,24}cible réelle|partie séparée|entrée dans l’organisme|entree dans l.organisme|germe|exposition|infection|contamination)/i.test(lower)) return "regle_mecanique";
...
function decisionModel(kind: TaxonomyKind, observation: string, title: string): { question: string; options: Array<{ key: string; label: string }>; impact: string } {
  ...
  switch (kind) {
    ...
    case "identite_doublon": {
      const distinct = /(distinct|distincte|distincts|ne pas assimiler|pas synonymes|pas fusionner|pas automatiquement|séparé|séparées|séparés)/i.test(observation);
      return distinct
        ? { question: `Le constat ci-dessus doit-il confirmer que les éléments cités restent distincts dans « ${title} » ?`, options: [{ key: "A", label: "Oui : conserver des fiches, rôles ou catégories distinctes ; ne créer aucun alias automatique." }, { key: "B", label: "Non : ils désignent finalement la même entrée ou un alias, après vérification." }, { key: "C", label: "Distinction conditionnelle : les relier seulement pour le contexte explicitement indiqué." }], impact: "Évite les fusions et liens automatiques entre éléments que le corpus décrit comme distincts." }
        : { question: `Le rapprochement signalé dans « ${title} » doit-il être traité comme une même identité, un alias ou des entrées distinctes ?`, options: [{ key: "A", label: "Même identité / alias : relier sous une fiche canonique après vérification." }, { key: "B", label: "Entrées distinctes : ne pas fusionner ni créer de lien d’identité." }, { key: "C", label: "Lien conditionnel : conserver le rapprochement uniquement dans le contexte indiqué." }], impact: "Identifiants, liens croisés, mini-fiches et risque de révélation MJ." };
    }
    case "chronologie":
