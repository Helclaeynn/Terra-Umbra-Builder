import {NPC_TIERS,NPC_TALENTS} from './npc-rules.js';
export {NPC_TIERS,NPC_TALENTS};
...
export function npcBudgetIssues(d:NpcData){
 ...
 if(d.talentIds.some(id=>!NPC_TALENTS.some(t=>t.id===id)||!npcTalentEligible(id,d.skills)))issues.push('Un talent ne remplit plus ses prérequis de compétence.');
 return issues;
}
export function validNpcData(value:unknown,catalog:Pick<NpcCatalog,'attributes'|'skills'>):value is NpcData{
 ...
 if(!Array.isArray(d.talentIds)||d.talentIds.length>NPC_TALENTS.length||new Set(d.talentIds).size!==d.talentIds.length)return false;
