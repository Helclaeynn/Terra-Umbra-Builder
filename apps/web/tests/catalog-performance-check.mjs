...
const {sortedNames,compareTruthTalents,truthGroups,createRenderCache,api,createWikiLinker}=await import('data:text/javascript;base64,'+Buffer.from(built.outputFiles[0].text).toString('base64'));
...
assert.deepEqual(truthGroups([{name:'Zénith',group:'Z',cost:1},{name:'B',group:'A',cost:10},{name:'Z',group:'A',cost:2},{name:'A',group:'A',cost:2}].sort(compareTruthTalents)).map(g=>[g.name,g.items.map(t=>t.name)]),[['A',['A','Z','B']],['Z',['Zénith']]]);
