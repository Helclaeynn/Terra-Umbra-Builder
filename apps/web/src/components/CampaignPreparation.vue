...
const props=defineProps<{modelValue:CampaignScene[];readonly?:boolean;play?:boolean;campaignReferences?:Article[]}>();
...
const reused=computed<Article[]>(()=>[...new Map([...(props.campaignReferences||[]),...props.modelValue.flatMap(s=>s.references).map(r=>({id:r.articleId,title:r.title,category:r.category,snippet:r.notes}))].map(a=>[a.id,a])).values()]);
   ...
   <div class="scene-body">
    ...
    <template v-if="!readonly">
     ...
     <div v-if="active===s.id" class="picker">
      ...
      <div class="actions" role="group" aria-label="Origine des références"><button v-for="(label,key) in {explore:'Explorer',dossier:'Déjà dans la campagne',favorites:'Mes favoris',recent:'Mes lectures récentes'}" :key="key" type="button" :aria-pressed="tab===key" @click="tab=String(key)">{{ label }}</button></div>
      <p v-if="tab==='explore'&&searching" role="status">Recherche…</p><p v-else-if="error" role="alert">{{ error }}</p><p v-else-if="tab==='explore'&&!results.length">Aucun résultat. Essaie un terme plus court ou « Tout le Compendium ».</p><p v-else-if="tab!=='explore'&&!suggestions.length">Aucune référence ici pour ces critères.</p>
      <small v-if="tab==='explore'&&!searching">{{ total }} résultat(s) · {{ results.length }} affiché(s)</small>
      <div class="results-list" tabindex="0" role="region" aria-label="Pages à ajouter" @scroll="scrolled">
       <div v-for="a in tab==='explore'?results:suggestions" :key="a.id" class="result">
        <div><strong>{{ a.title }}</strong><small>{{ a.category }}{{ a.group?' · '+a.group:'' }}</small><p>{{ a.snippet }}</p><div class="tags"><button v-for="tag in (a.tags||[]).slice(0,5)" :key="tag" type="button" @click="filterTag(tag)">{{ tag }}</button></div><a :href="`/compendium?article=${encodeURIComponent(a.id)}`" target="_blank" rel="noopener noreferrer">Consulter avant d’ajouter ↗</a></div>
