...
const props=defineProps<{modelValue:CampaignScene[];campaignId?:string;readonly?:boolean;play?:boolean;campaignReferences?:Article[]}>();
const emit=defineEmits<{(e:'update:modelValue',v:CampaignScene[]):void;(e:'npc-dirty',value:boolean):void}>();
type Article={npcId?:string;id:string;title:string;category:string;snippet:string;tags?:string[];group?:string};
const openedNpc=ref(''),npcDirty=ref(false);
...
const reused=computed<Article[]>(()=>[...new Map([...(props.campaignReferences||[]),...props.modelValue.flatMap(s=>s.references).map(r=>({id:r.articleId,title:r.title,category:r.category,snippet:r.notes,npcId:r.npcId}))].map(a=>[a.id,a])).values()]);
...
function select(a:Article,sceneId:string){change(s=>{const scene=s.find(s=>s.id===sceneId);if(scene&&scene.references.length<30&&!scene.references.some(r=>r.articleId===a.id)){scene.references.push({articleId:a.id,title:a.title,category:a.category,quantity:1,notes:'',...(a.npcId?{npcId:a.npcId}:{})});addedNotice.value=`${a.title} ajouté à « ${scene.title} ».`;}});}
