   ...
   <div class="scene-body">
    ...
    <template v-if="!readonly">
     ...
     <div v-if="active===s.id" class="picker">
      <strong>Que te faut-il pour cette scène ?</strong>
      <div class="actions" role="group" aria-label="Type de référence"><button v-for="(label,key) in {Personnages:'Un PNJ',Bestiaire:'Une créature',Règles:'Une règle','':'Un lieu ou un dossier'}" :key="key" type="button" :aria-pressed="category===key" @click="category=String(key)">{{ label }}</button></div>
      ...
      <div v-for="a in tab==='explore'?results:suggestions" :key="a.id" class="result"><div><strong>{{ a.title }}</strong><small>{{ a.category }}{{ a.group?' · '+a.group:'' }}</small><p>{{ a.snippet }}</p><a :href="`/compendium?article=${encodeURIComponent(a.id)}`" target="_blank" rel="noopener noreferrer">Consulter avant d’ajouter ↗</a></div><button type="button" :disabled="has(a.id)||s.references.length>=30" :aria-label="`Ajouter ${a.title}`" @click="select(a)">{{ has(a.id)?'Ajouté à cette scène':'Ajouter à la scène' }}</button></div>
