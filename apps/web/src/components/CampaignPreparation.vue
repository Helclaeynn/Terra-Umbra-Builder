...
const difficulties=[{id:'figurant',label:'Figurant'},{id:'standard',label:'Standard'},{id:'dangereux',label:'Dangereux'},{id:'majeur',label:'Majeur'},{id:'exceptionnel',label:'Exceptionnel'}];
function creature(r:SceneReference){return Boolean(r.creatureId||r.category==='Bestiaire');}
function encounter(scene:CampaignScene){const weights:Record<string,number>={figurant:1,standard:2,dangereux:4,majeur:7,exceptionnel:12};const known=scene.references.filter(r=>creature(r)&&r.difficultyId&&weights[r.difficultyId]);const unknown=scene.references.filter(r=>creature(r)&&!r.difficultyId).reduce((n,r)=>n+r.quantity,0);if(!known.length)return unknown?`${unknown} créature(s) à chiffrer pour estimer la rencontre`:'';const total=known.reduce((n,r)=>n+weights[r.difficultyId!]*r.quantity,0),size=Math.max(1,props.groupSize||4);const level=total<=size?'Légère':total<=size*2?'Soutenue':total<=size*3?'Périlleuse':'Extrême';return `${level} · ${known.reduce((n,r)=>n+r.quantity,0)} créature(s) chiffrée(s)${unknown?` · ${unknown} créature(s) encore à chiffrer`:''}${scene.references.some(r=>r.npcId)?' · PNJ non comptés':''}`;}
  ...
  <details v-for="(s,index) in modelValue" :key="s.id" class="scene" :open="readonly||expanded.includes(s.id)" @toggle="($event.target as HTMLDetailsElement).open ? !expanded.includes(s.id)&&expanded.push(s.id) : expanded=expanded.filter(id=>id!==s.id)">
   ...
   <div class="scene-body">
    ...
    <div v-for="r in s.references" :key="r.articleId" class="reference">
     ...
     <CampaignNpcPreview v-if="r.npcId&&campaignId&&openedNpc===r.npcId" :key="r.npcId" :campaign-id="campaignId" :npc-id="r.npcId" /><CampaignCreaturePreview v-if="r.creatureId&&campaignId&&openedCreature===r.creatureId" :key="r.creatureId" :campaign-id="campaignId" :creature-id="r.creatureId" /><details v-if="!readonly" class="reference-options"><summary>Quantité et annotation</summary><label>Quantité<input type="number" :value="r.quantity" min="1" max="999" required @input="patchRef(s.id,r.articleId,{quantity:Number(value($event))})" /></label><label v-if="creature(r)">Difficulté estimée pour {{ r.title }}<select :value="r.difficultyId||''" @change="patchRef(s.id,r.articleId,{difficultyId:value($event)||undefined})"><option value="">À préciser</option><option v-for="tier in difficulties" :key="tier.id" :value="tier.id">{{ tier.label }}</option></select><small>Pour une créature du Compendium ou une ancienne fiche, choisis le palier avec le MJ selon son profil réel en scène.</small></label><label>Annotation privée<textarea :value="r.notes" rows="2" maxlength="2000" placeholder="Rôle dans la scène, particularités, informations à révéler…" @input="patchRef(s.id,r.articleId,{notes:value($event)})" /></label><button type="button" :aria-label="`Retirer ${r.title}`" @click="removeRef(s.id,r.articleId)">Retirer la référence</button></details><template v-else><small v-if="creature(r)">Difficulté : {{ difficulties.find(tier=>tier.id===r.difficultyId)?.label||'À préciser' }}</small><p v-if="r.notes" class="prose">{{ r.notes }}</p></template>
    </div>
