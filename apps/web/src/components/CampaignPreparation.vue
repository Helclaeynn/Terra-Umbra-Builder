...
type Article={npcId?:string;creatureId?:string;id:string;title:string;category:string;snippet:string;tags?:string[];group?:string};
...
function selectCreature(c:BestiarySummary,id:string){select({id:'campaign-creature:'+c.id,creatureId:c.id,title:c.name,category:'Bestiaire de campagne',snippet:c.role},id);}
...
const reused=computed<Article[]>(()=>[...new Map([...(props.campaignReferences||[]),...props.modelValue.flatMap(s=>s.references).map(r=>({id:r.articleId,title:r.title,category:r.category,snippet:r.notes,npcId:r.npcId,creatureId:r.creatureId}))].map(a=>[a.id,a])).values()]);
...
function select(a:Article,sceneId:string){change(s=>{const scene=s.find(s=>s.id===sceneId);if(scene&&scene.references.length<30&&!scene.references.some(r=>r.articleId===a.id)){scene.references.push({articleId:a.id,title:a.title,category:a.category,quantity:1,notes:'',...(a.npcId?{npcId:a.npcId}:{}),...(a.creatureId?{creatureId:a.creatureId}:{})});addedNotice.value=`${a.title} ajouté à « ${scene.title} ».`;}});}
   ...
   <div class="scene-body">
    ...
    <template v-if="!readonly">
     ...
     <div v-if="active===s.id" class="picker">
      ...
      <CampaignBestiary v-if="tab==='bestiary'&&campaignId" :campaign-id="campaignId" selectable :selection-full="s.references.length>=30" :selected-ids="s.references.map(r=>r.creatureId||'')" @select="selectCreature($event,s.id)" @dirty="npcChanged" />
