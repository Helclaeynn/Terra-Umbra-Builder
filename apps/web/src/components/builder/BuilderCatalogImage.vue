...
const props=defineProps<{articleId?:string;name:string;category?:string;generation?:number|null}>();
...
async function resolve(variant=true){const key=`${props.articleId||`${props.category||''}:${props.name}`}:${variant?props.generation??'':''}`;if(!key)return '';let pending=cache.get(key);if(!pending){pending=(async()=>{let id=props.articleId;if(!id){const params=new URLSearchParams({q:props.name,limit:'8'});if(props.category)params.set('category',props.category);const result=await api<{items:{id:string;title:string}[]}>(`/api/compendium/search?${params}`);id=result.items.find(row=>row.title.toLocaleLowerCase('fr')===props.name.toLocaleLowerCase('fr'))?.id;}if(!id)return '';if(variant&&[1,2].includes(props.generation??0)&&/^augmentation-\d{3}-[a-z0-9-]+$/.test(id))return `/api/compendium/media/images/manual/${id}--g${props.generation}.webp`;const result=await api<{media:unknown}>(`/api/compendium/wiki-preview/${encodeURIComponent(id)}`);return media(result.media);})().catch(()=> '');cache.set(key,pending);}return pending;}
...
onMounted(observe);onUnmounted(()=>{alive=false;request++;observer?.disconnect();});watch(()=>[props.articleId,props.name,props.category,props.generation],()=>{request++;src.value='';observe();});
</script>
