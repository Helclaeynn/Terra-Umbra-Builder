...
<template>
  <article class="progression-step">
    ...
    <details class="progress-panel" open>
    ...
    <section class="progress-panel">
      ...
      <details class="trade-block" open>
        ...
        <div class="trade-grid">
          ...
          <label class="wide">Article<select v-model="tradeItemId" @change="selectTradeItem"><option value="">— Choisir —</option><option v-for="item in tradeRows" :key="item.id" :value="item.id">{{ item.name }} · {{ realityPriceSpec(item).label }}</option></select></label>
