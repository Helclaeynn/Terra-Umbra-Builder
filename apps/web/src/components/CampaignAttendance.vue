...
<template>
 <section class="attendance" aria-label="Présences à la séance">
  ...
  <details><summary>Voir les réponses{{ canManage?' / ajuster les présences':'' }}</summary><div v-for="r in rows" :key="r.userId" class="row"><span>{{ r.displayName }}</span><label v-if="canManage&&!archived"> <span class="sr-only">Présence de {{ r.displayName }}</span><select :aria-label="`Présence de ${r.displayName}`" :value="r.response||''" :disabled="busy" @change="respond(r.userId,($event.target as HTMLSelectElement).value)"><option value="" disabled>Sans réponse</option><option v-for="(label,key) in labels" :key="key" :value="key">{{ label }}</option></select></label><strong v-else>{{ r.response?labels[r.response]:'Sans réponse' }}</strong></div></details>
 </section>
</template>
<style scoped>
