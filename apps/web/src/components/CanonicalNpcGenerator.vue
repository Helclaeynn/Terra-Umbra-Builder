...
const catalog=ref<NpcCatalog|null>(null),draft=ref<NpcData|null>(null),tierId=ref('lambda'),presetId=ref('garde'),sex=ref('random'),faction=ref(''),busy=ref(false),imageBusy=ref(false),error=ref('');
...
async function generate(){if(locked.value||draft.value&&!window.confirm('Remplacer cet aperçu et ses modifications ?'))return;busy.value=true;error.value='';try{const r=await api<{npcs:NpcData[]}>(endpoint+'/generate',{method:'POST',body:JSON.stringify({tierId:tierId.value,presetId:presetId.value,sex:sex.value,faction:faction.value,count:1,seed:crypto.randomUUID()})});if(live){draft.value=r.npcs[0];applied=false;}}catch{if(live)error.value='Impossible de générer ce PNJ. Réessaie.';}finally{busy.value=false;}}
 ...
 <section class="canonical-generator" aria-label="Générateur de PNJ canonique">
  ...
  <template v-if="catalog">
   <fieldset :disabled="locked"><div class="choices">
    ...
    <label>Faction ou groupe suggéré<input v-model="faction" maxlength="160" placeholder="Facultatif · bloc MJ" /></label>
   </div><p>{{ catalog.presets.find(p=>p.id===presetId)?.description }}</p><button type="button" @click="generate">{{ draft?'Générer un autre aperçu':'Générer un aperçu' }}</button></fieldset>
