Warning: truncated output (original token count: 39038)
Total output lines: 3233
...
const hasRenownContactTalent=computed(()=>selectedRealityTalentIds().includes("contact_de_renom"));
...
function selectedNpcContact(value:unknown){
...
function handleBuilderKeydown(event:KeyboardEvent){
  ...
  if(event.shiftKey&&document.activeElement===first){event.preventDefault();last?.…9038 tokens truncated…>
                  <h3>
                    <BuilderWikiLink
