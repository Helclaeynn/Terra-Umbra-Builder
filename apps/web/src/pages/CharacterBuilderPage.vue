...
<template>
  <div class="builder-v2-shell">
    ...
    >
      <header class="knowledge-head">
        <div>
          ...
          <p>Les pages de référence liées au personnage actuel, sans quitter la création.</p>
        </div>
      ...
      <div v-if="!knowledgeRefs.length" class="knowledge-empty">
        Fais quelques choix dans le Builder : leurs références apparaîtront ici automatiquement.
      </div>
    ...
    <main v-else-if="draft && character && rules && lore" class="builder-workspace">
        ...
        <article v-else-if="activeStep === 'truth'" class="panel builder-card">
          ...
          <template v-else>
            ...
            <template v-if="selectedTruthNature">
              ...
              <section v-if="truthRevealProfile" class="truth-reveal-section">
                <div class="subsection-title">
                  <div>
                    <h3>Voile & Révélation</h3>
                    <p>
                  ...
                  <span class="schema-badge">{{ truthRevealProfile.label }}</span>
                </div>
      ...
      <aside class="builder-live-summary">
        <div class="panel live-summary-card">
          <header class="live-summary-head">
            <span>Résumé vivant</span>
            <span class="live-summary-badge" :class="{ good: stepDone('finish') }">
              {{ stepDone("finish") ? "Valide" : "À compléter" }}
            </span>
          </header>

          <section class="live-summary-section">
            <h3>Personnage</h3>
            <div class="live-summary-line"><span>Nom</span><strong>{{ identityDisplayName || "—" }}</strong></div>
            <div class="live-summary-line"><span>Origine</span><strong>{{ originNameValue || "—" }}</strong></div>
            <div class="live-summary-line"><span>Sphère</span><strong>{{ sphereNameValue || "—" }}</strong></div>
            <div class="live-summary-line"><span>Style</span><strong>{{ styleNameValue || "—" }}</strong></div>
          </section>

          <section class="live-summary-section">
            <h3>Ressources</h3>
            <div class="live-summary-line">
              <span>Train de vie</span>
              <strong>{{ lifestylePressureValue?.effective || lifestyleBaseValue || "—" }}</strong>
            </div>
            <div class="live-summary-line"><span>Compte</span><strong>{{ formatMoney(realityEconomyValue?.account || 0) }}</strong></div>
            <div class="live-summary-line"><span>Edge</span><strong>{{ edgeRemaining }} / {{ edgeTotal }}</strong></div>
            <div class="live-summary-line"><span>PTV</span><strong>{{ truthPtvRemaining }}</strong></div>
            <div class="live-summary-line"><span>Renommée</span><strong>{{ renownScore }}</strong></div>
          </section>

          <section class="live-summary-section">
            <h3>Dérivés</h3>
            <div class="live-summary-line"><span>PV</span><strong>{{ derivedStats.pvMax }}</strong></div>
            <div class="live-summary-line"><span>Initiative</span><strong>+{{ derivedStats.initiative }}</strong></div>
            <div class="live-summary-line"><span>Défense</span><strong>{{ derivedStats.passiveDefense }}</strong></div>
            <div class="live-summary-line"><span>Intégrité</span><strong>{{ derivedStats.integrity }}</strong></div>
          </section>

          <section class="live-summary-section compact-summary">
            <h3>Création</h3>
            <div class="live-summary-line"><span>Étapes</span><strong>{{ builderProgress.done }}/{{ builderProgress.total }}</strong></div>
            <div class="live-summary-line"><span>Équipement</span><strong>{{ equipmentCount }}</strong></div>
            <div class="live-summary-line"><span>Augmentations</span><strong>{{ augmentationCount }}</strong></div>
          </section>
        </div>
      </aside>
    </main>
...
.knowledge-head,.knowledge-group{border-color:var(--tu-border-soft)}
.knowledge-group h3{color:var(--tu-accent)}
.builder-workspace{
...
.builder-character{
  ...
  background:linear-gradient(135deg,rgba(72,215,177,.055),transparent);
}
.builder-character h1{
  font-family:Inter,Segoe UI,Roboto,Arial,sans-serif;
  font-weight:750;
  letter-spacing:-.02em;
}
.builder-mini-portrait{
  border-color:#344550;
  background:#080e13;
  border-radius:10px;
}
.builder-mini-portrait.empty span{color:var(--tu-accent)}
.builder-progress{
  border-bottom-color:var(--tu-border-soft);
  background:#0d141a;
}
.builder-progress-head{color:var(--tu-muted)}
...
.builder-progress-track span{
  ...
  background:linear-gradient(90deg,var(--tu-accent),var(--tu-accent2));
}
.builder-progress small{color:#71818d}
.builder-nav{
  gap:6px;
  padding:.65rem;
}
...
.builder-nav button:hover:not(:disabled){
  color:var(--tu-text);
  background:#121b23;
}
...
.builder-nav button.done:not(.active){color:#b8c8d0}
.builder-nav button.done:not(.active)::after{
  background:var(--tu-success);
}
.builder-nav button small{color:#667985}
.builder-nav button.done small{color:#77b68a}
.builder-card{
  min-height:680px;
  border-color:var(--tu-border);
  background:rgba(17,24,32,.94);
...
.builder-heading h2{letter-spacing:-.025em}
.schema-badge{
  border-color:#344550;
  border-radius:10px;
  color:#b9c7cf;
...
.builder-intro,.field-help,.subsection-title p{color:var(--tu-muted)}
textarea{
  border-color:var(--tu-border);
  border-radius:10px;
  color:var(--tu-text);
...
textarea:focus{
  border-color:#456574;
  box-shadow:0 0 0 2px rgba(72,215,177,.10);
}
.choice-card,
.allocator-card,
.attribute-card,
...
.choice-card:hover,
.truth-talent-card:hover:not(:disabled){
  border-color:#46616e;
}
.choice-card.selected,
.truth-talent-card.selected{
  border-color:var(--tu-accent);
  background:#10201f;
  box-shadow:inset 0 0 0 1px rgba(72,215,177,.45);
}
.choice-card span,
.choice-card small,
.allocator-card p,
.attribute-card p{color:var(--tu-muted)}
.stepper button{
  border-color:var(--tu-border);
  border-radius:8px;
  color:var(--tu-text);
...
.stepper button:hover{border-color:#46616e}
.rule-note{
  border-color:#28414a;
  border-radius:12px;
  color:#b8cad2;
...
.truth-talent-head span,
.truth-talent-wiki,
.choice-card-wiki{color:var(--tu-accent)}
.live-summary-card{
  padding:16px;
  border-color:var(--tu-border);
  background:rgba(17,24,32,.94);
...
.live-summary-badge{
  padding:4px 8px;
  border:1px solid #603039;
  border-radius:999px;
  color:var(--tu-danger);
  background:#261116;
  font-size:.68rem;
}
.live-summary-badge.good{
...
.live-summary-section:last-child{border-bottom:0}
...
.live-summary-line strong{
  color:var(--tu-text);
  text-align:right;
  font-weight:750;
}
.compact-summary .live-summary-line{font-size:.72rem}
@media(max-width:1250px){
  .builder-workspace{grid-template-columns:230px minmax(0,1fr)}
  .builder-live-summary{grid-column:1/-1}
  .live-summary-card{
    position:static;
    display:grid;
  ...
  .live-summary-head{grid-column:1/-1}
  .live-summary-section{border-bottom:0}
}
@media(max-width:820px){
  .builder-workspace{grid-template-columns:1fr}
  .builder-sidebar{position:static}
  .live-summary-card{grid-template-columns:1fr 1fr}
}
@media(max-width:560px){
  .live-summary-card{grid-template-columns:1fr}
}
