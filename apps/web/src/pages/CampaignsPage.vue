...
type Campaign={id:string;name:string;description:string;gmName:string;ownerId:string;canManage:boolean;membershipStatus:'invited'|'accepted'|null;memberCount?:number;archivedAt:string|null;version:number;gmNotes?:string};
type Member={userId:string;displayName:string;status:string;characterId:string|null;characterName:string|null;canReadSheet:boolean;updatedAt:string|null};
type Account={id:string;displayName:string};
const router=useRouter(),id=String(useRoute().params.id||''),endpoint='/api/campaigns';
const campaigns=ref<Campaign[]>([]),campaign=ref<Campaign|null>(null),members=ref<Member[]>([]),characters=ref<{id:string;name:string}[]>([]);
...
const chosenCharacter=ref(''),editing=ref(false),draft=ref({name:'',description:'',gmNotes:''}),baseline=ref(''),editVersion=ref(0);
...
function failure(e:unknown){
  ...
  if(e instanceof ApiError&&[401,403,404].includes(e.status)){
    campaign.value=null;campaigns.value=[];members.value=[];characters.value=[];accounts.value=[];canCreate.value=false;editing.value=false;draft.value={name:'',description:'',gmNotes:''};login.value=e.status===401;
  }
}
async function load(){
...
watch(query,()=>{
  const seq=++searchGeneration;clearTimeout(timer);accounts.value=[];searchError.value='';searching.value=false;
  const q=query.value.trim();if(q.length<2||q.length>80)return;
  searching.value=true;timer=setTimeout(async()=>{
    try{const r=await api<{accounts:Account[]}>(`${endpoint}/${id}/accounts?q=${encodeURIComponent(q)}`);if(seq===searchGeneration)accounts.value=r.accounts;}
    ...
    finally{if(seq===searchGeneration)searching.value=false;}
  },250);
});
async function invite(a:Account){await action(async()=>{await api(`${endpoint}/${id}/invitations`,{method:'POST',body:JSON.stringify({userId:a.id})});query.value='';},`${a.displayName} a été invité. L’invitation apparaît dans ses campagnes.`);}
async function join(){await action(()=>api(`${endpoint}/${id}/membership`,{method:'PUT',body:JSON.stringify({characterId:chosenCharacter.value||null})}),'Participation enregistrée.');}
...
function edit(){const c=campaign.value!;draft.value={name:c.name,description:c.description,gmNotes:c.gmNotes||''};baseline.value=JSON.stringify(draft.value);editVersion.value=c.version;editing.value=true;}
      ...
      <template v-else-if="campaign">
        ...
        <template v-if="campaign.canManage||campaign.membershipStatus==='accepted'">
          <section class="panel"><div class="section-heading"><h2>Le groupe</h2><span>{{ members.filter(m=>m.status==='accepted').length }} joueur(s)</span></div><p v-if="!members.length" class="empty">Invite tes joueurs pour réunir leurs fiches ici.</p>
            <div v-for="m in members" :key="m.userId" class="member-row"><div><strong>{{ m.characterName||m.displayName }}</strong><p>{{ m.displayName }} <span v-if="m.status==='invited'">· Invitation en attente</span><span v-else-if="!m.characterId">· Personnage à choisir</span></p><small v-if="m.updatedAt">Fiche mise à jour le {{ new Date(m.updatedAt).toLocaleDateString('fr-FR') }}</small></div><div class="actions"><RouterLink v-if="m.canReadSheet" class="primary sheet-link" :to="{path:`/characters/${m.characterId}/sheet`,query:{campaign:id}}">Ouvrir la fiche →</RouterLink><button v-if="campaign.canManage&&!campaign.archivedAt" :disabled="busy" :aria-label="`Retirer ${m.displayName}`" @click="remove(m)">{{ m.status==='invited'?'Annuler l’invitation':'Retirer' }}</button></div></div>
          </section>
          <section v-if="!campaign.canManage&&me" class="panel form"><h2>Mon personnage</h2><p>La fiche choisie est partagée en lecture avec le MJ. Les autres joueurs ne peuvent pas l’ouvrir.</p><label>Fiche partagée<select v-model="chosenCharacter"><option value="">Aucune fiche partagée</option><option v-for="c in characters" :key="c.id" :value="c.id">{{ c.name }}</option></select></label><div class="actions"><button class="primary" :disabled="busy||chosenCharacter===(me.characterId||'')" @click="join">Enregistrer mon choix</button><button :disabled="busy" @click="remove(me)">Quitter la campagne</button></div><RouterLink v-if="!characters.length" to="/account">Créer un personnage dans Mon espace →</RouterLink></section>
          <section v-if="campaign.canManage&&!campaign.archivedAt" class="panel form"><h2>Inviter un joueur</h2><label>Nom de compte<input v-model="query" type="search" maxlength="80" placeholder="Au moins 2 caractères…" autocomplete="off" /></label><p v-if="searching" role="status">Recherche…</p><p v-else-if="searchError" role="alert">{{ searchError }}</p><p v-else-if="query.trim().length>=2&&!accounts.length" role="status">Aucun compte disponible. Les joueurs déjà invités ne sont pas proposés.</p><div v-for="a in accounts" :key="a.id" class="member-row"><div><strong>{{ a.displayName }}</strong><small>Compte {{ a.id.slice(0,8) }}</small></div><button :disabled="busy" @click="invite(a)">Inviter {{ a.displayName }}</button></div><small>Le joueur recevra l’invitation dans « Mes campagnes » et choisira lui-même sa fiche.</small></section>
          <CampaignSessions :campaign-id="id" :can-manage="campaign.canManage" :archived="!!campaign.archivedAt" :members="members" />
