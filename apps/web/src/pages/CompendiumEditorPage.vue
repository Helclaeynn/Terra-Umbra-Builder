          ...
          <section class="editor-form-column">
            <div class="panel editor-card">
              ...
              <div class="editor-two">
                <label>Statut<select v-model="article.status" aria-label="Statut"><option v-if="article.status&&!editorialStatuses.some(item=>item.id===article?.status)" :value="article.status">{{ article.status }} · statut existant</option><option v-for="item in editorialStatuses" :key="item.id" :value="item.id">{{ item.label }}</option></select></label>
