(()=>{
  ...
  const DB=window.ANGELUS_PREVIEW||{pages:[]};
  ...
  function boot(){
    try{
      if(!Array.isArray(DB.pages)||DB.pages.length!==21)throw new Error(`Preview invalide : ${DB.pages?.length??0} pages chargées au lieu de 21.`);
      nav();render();$('#filter').addEventListener('input',filter);window.addEventListener('hashchange',render);
    }catch(e){console.error(e);$('#content').innerHTML=`<div class="notice">Impossible de charger la preview Arbre de Vie & Angelus : ${esc(e.message||e)}</div>`}
  }
  boot();
})();