...
export async function loadPnjExtensions(existingMeta=[]){const titles=new Set(existingMeta.map(a=>norm(a.title)));const seeds=await loadSeeds(titles);const idx=await loadIndex(titles);return {articles:[...seeds,...idx.articles],duplicates:idx.duplicates,indexError:idx.error}}
