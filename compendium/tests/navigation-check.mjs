...
const minimumCounts={'Règles':251,'Réalité':9,'Équipement':357,'Augmentations':111,'Catalogue Vérité':229,'Organisations':304,'Personnages':223,'Bestiaire':263};
for(const [category,count] of Object.entries(minimumCounts))if(expectedRawCounts[category]<count)throw new Error(`${category}: ${expectedRawCounts[category]}, minimum ${count} — aucune entrée ne doit disparaître pendant la restructuration`);
