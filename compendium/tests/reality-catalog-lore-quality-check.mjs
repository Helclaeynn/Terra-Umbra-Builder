...
const all=[];const byTitle=new Map();let sourceDriven=0,derived=0,augmentations=0;const pageErrors=[];
for(const id of IDS){
  ...
  for(const page of load(spec)){
    ...
    const problems=[];
    if(blocks.length!==2)problems.push(`${blocks.length} paragraphes, attendu 2`);
    if(page.catalog?.loreVersion!==3||page.catalog?.loreMethod!=='reality-book-semantic-lore')problems.push('traçabilité lore V3 absente');
    if(text.length<150)problems.push(`contexte trop pauvre (${text.length})`);
    if(/\$/.test(text))problems.push('le lore répète encore un prix');
...
if(pageErrors.length)throw new Error(`Lore Réalité V3 invalide (${pageErrors.length} pages) — ${pageErrors.join(' | ')}`);
