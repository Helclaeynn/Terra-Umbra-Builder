...
const expectedCounts={moteur:5,realite:39,verite:63,bestiaire:262,lore:397,pnj:163};
...
for(const spec of manifest.datasets){
  ...
  for(const [i,row] of rows.entries()){
    ...
    if(spec.id==='bestiaire'){
      ...
      const description=sections.find(section=>section.id==='description');
      const dossierMj=sections.find(section=>section.id==='dossier-mj');
