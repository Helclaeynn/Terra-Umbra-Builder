...
const expectedCounts={moteur:5,realite:39,verite:63,bestiaire:131,lore:397,pnj:163};
...
for(const spec of manifest.datasets){
  ...
  for(const [i,row] of rows.entries()){
    ...
    if(spec.id==='bestiaire'){
      if(row.category!=='Bestiaire') throw new Error(`bestiaire[${i}]: catégorie invalide`);
      if(!row.bestiary?.chapter) throw new Error(`bestiaire[${i}]: chapitre absent`);
      if(!row.bestiary?.hook) throw new Error(`bestiaire[${i}]: accroche absente`);
      if(!row.illustration?.src) throw new Error(`bestiaire[${i}]: emplacement illustration absent`);
      const ids=new Set((row.sections||[]).map(section=>section.id));
      for(const required of ['description','accroche','profil']) if(!ids.has(required)) throw new Error(`bestiaire[${i}]: section ${required} absente`);
