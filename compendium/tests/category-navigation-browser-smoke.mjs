...
async function openCategory(category,expectedCards,minGroups){
  ...
  if(count!==expectedCards)throw new Error(`${category}: ${count} cartes hiérarchiques, attendu ${expectedCards}`);
...
try{
  const rules=await openCategory('Règles',247,9);
  ...
  const reality=await openCategory('Réalité',39,5);
  ...
  if(filtered<1||filtered>=39)throw new Error(`Réalité: filtre Neurodive incohérent (${filtered})`);

  const truth=await openCategory('Vérité',128,7);
