...
async function exactTitleCard(title,prefix){const escaped=title.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');const heading=page.locator(`.article-card[href^="#/article/${prefix}-"]:visible h3`).filter({hasText:new RegExp(`^${escaped}$`)});const count=await heading.count();if(count!==1){const candidates=await visibleCardSnapshot(prefix);throw new Error(`${title}: ${count} titre(s) exact(s) dans ${prefix}, attendu 1 · cartes visibles=${JSON.stringify(candidates)}`);}return heading.first().locator('..');}
...
async function openExact(title,needles=[]){await gotoCategory(CATALOG_CATEGORY);await filterCategory(title);const card=await exactTitleCard(title,'equipement');const href=await card.getAttribute('href');if(!href?.startsWith('#/article/equipement-'))throw new Error(`${title}: route canonique équipement absente`);await card.click();const text=await articleLore(title);const hash=await page.evaluate(()=>location.hash);if(hash!==href)throw new Error(`${title}: route ouverte ${hash}, attendu ${href}`);for(const needle of needles){const n=needle.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();if(!text.includes(n))throw new Error(`${title}: détail attendu absent (${needle})`);}return text;}
