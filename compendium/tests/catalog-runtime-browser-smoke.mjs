...
async function exactTitleCard(title,prefix){const heading=page.locator(`.article-card[href^="#/article/${prefix}-"]:visible h3`).filter({hasText:new RegExp(`^${title.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}$`)});const count=await heading.count();if(count!==1)throw new Error(`${title}: ${count} titre(s) exact(s) dans ${prefix}, attendu 1`);return heading.first().locator('..');}
