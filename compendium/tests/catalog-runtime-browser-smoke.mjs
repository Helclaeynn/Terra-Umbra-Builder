...
async function openExact(title,needles=[]){await gotoCategory(CATALOG_CATEGORY);await filterCategory(title);const card=await exactTitleCard(title);await card.click();const text=await articleLore(title);for(const needle of needles){const n=needle.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();if(!text.includes(n))throw new Error(`${title}: détail attendu absent (${needle})`);}return text;}
...
try{
  ...
  await gotoCategory(CATALOG_CATEGORY);await filterCategory('Neuroprogramme');const neuroCount=await visibleCards().count();if(neuroCount!==27)throw new Error(`Neuroprogrammes: ${neuroCount} cartes, attendu 27`);
  if(errors.length)throw new Error(`Erreurs navigateur:\n${errors.join('\n')}`);
  console.log(`Browser smoke Réalité V3 OK — rubrique unifiée Équipement & Objets · ${augmentationCount} pages d’augmentations · Vladic/Cache facts-only · ${neuroCount} Neuroprogrammes · aucun asset jsDelivr.`);
