...
function visibleCards(prefix=''){const scope=prefix?`[href^="#/article/${prefix}-"]`:'';return page.locator(`.article-card${scope}:visible`);}
async function visibleCardSnapshot(prefix){return page.locator(`.article-card[href^="#/article/${prefix}-"]:visible`).evaluateAll(cards=>cards.slice(0,20).map(card=>({title:card.querySelector('h3')?.textContent?.trim()||'',href:card.getAttribute('href')||'',filter:card.getAttribute('data-filter')||''})));}
...
async function exactTitleCard(title,prefix,{filterNeedle='',allowQualifiedTitle=false}={}){const escaped=title.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');const qualified=allowQualifiedTitle?'(?:\\s+—\\s+.+)?':'';const filterScope=filterNeedle?`[data-filter*="${filterNeedle}"]`:'';const heading=page.locator(`.article-card[href^="#/article/${prefix}-"]${filterScope}:visible h3`).filter({hasText:new RegExp(`^${escaped}${qualified}$`)});const count=await heading.count();if(count!==1){const candidates=await visibleCardSnapshot(prefix);throw new Error(`${title}: ${count} titre(s) canonique(s) dans ${prefix}${filterNeedle?` avec ${filterNeedle}`:''}, attendu 1 · cartes visibles=${JSON.stringify(candidates)}`);}return heading.first().locator('..');}
...
async function openExact(title,needles=[],options={}){await gotoCategory(CATALOG_CATEGORY);await filterCategory(title);const card=await exactTitleCard(title,'equipement',options);const href=await card.getAttribute('href');if(!href?.startsWith('#/article/equipement-'))throw new Error(`${title}: route canonique équipement absente`);await card.click();const text=await articleLore(title);const hash=await page.evaluate(()=>location.hash);if(hash!==href)throw new Error(`${title}: route ouverte ${hash}, attendu ${href}`);for(const needle of needles){const n=needle.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();if(!text.includes(n))throw new Error(`${title}: détail attendu absent (${needle})`);}return text;}

try{
  await gotoCategory(CATALOG_CATEGORY);
  await filterCategory('Augmentations');
  const augmentationCount=await visibleCards('augmentation').count();
  if(!augmentationCount||augmentationCount>=146)throw new Error(`Augmentations: ${augmentationCount} pages Réalité visibles pour 146 variantes runtime`);
  await filterCategory('Cybermain');const cyberCards=visibleCards('augmentation');if(await cyberCards.count()!==1)throw new Error(`Cybermain: ${await cyberCards.count()} cartes Réalité, attendu 1`);await cyberCards.first().click();await articleLore('Cybermain');
  ...
  await openExact('2-Fence',['deux agents humanoides','interception','dix minutes','reinitialisation'],{filterNeedle:'neuroprogramme defensif',allowQualifiedTitle:true});
  ...
  console.log(`Browser smoke Réalité V3 OK — rubrique unifiée Équipement & Objets · ${augmentationCount} pages d’augmentations Réalité · routes canoniques équipement validées · Vladic/Cache facts-only · ${neuroCount} Neuroprogrammes · aucun asset jsDelivr.`);
