(()=>{
  ...
  async function decode(){
    const b=atob(window.CORPORATIONS_PREVIEW_GZ||''),bytes=Uint8Array.from(b,c=>c.charCodeAt(0));
  ...
  async function boot(){
    try{
      DB=await decode();
      if(!Array.isArray(DB.pages)||DB.pages.length!==68)throw new Error(`Preview invalide : ${DB.pages?.length??0} pages chargées au lieu de 68.`);
      nav();render();$('#filter').addEventListener('input',filter);window.addEventListener('hashchange',render);
    }catch(e){console.error(e);$('#content').innerHTML=`<div class="notice">Impossible de charger la preview Corporations : ${esc(e.message||e)}</div>`}
  }
  boot();
})();