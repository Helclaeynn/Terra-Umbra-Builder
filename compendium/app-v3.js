...
function renderNav(active=''){
  ...
  nav.innerHTML=`<a href="#/start" class="${active==='start'?'active':''}">Nouveau joueur</a><a href="#/home" class="${active==='home'?'active':''}">Index du Compendium</a><hr>${manifest.categories.map(c=>`<a href="#/category/${encodeURIComponent(c)}" class="${active===c?'active':''}"><span>${esc(c)}</span><span class="count">${counts[c]||0}</span></a>`).join('')}<hr><a href="#/search" class="${active==='search'?'active':''}">Recherche globale</a><a href="#/media-audit" class="media-audit-link ${active==='media-audit'?'active':''}">Audit médias</a>`;
}
...
function metaBadges(a){return `<div class="meta"><span class="badge ${statusClass(a.status)}">${esc(statusLabel(a.status))}</span>${a.__editorialOverride?'<span class="badge canon">Édité</span>':''}${a.audience==='mj'?'<span class="badge mj">Contenu MJ</span>':''}${a.pnj?.completeness?`<span class="badge pnj-state ${esc(a.pnj.completeness)}">${esc(pnjCompletenessLabel(a.pnj.completeness))}</span>`:''}${(a.tags||[]).filter(Boolean).slice(0,8).map(t=>`<a class="badge tag-link" href="${tagHref(a,t)}" title="Explorer les entrées liées à ce tag">${esc(t)}</a>`).join('')}</div>`}
...
function blockHtml(b,currentArticle=''){if(b.type==='table')return `<div class="doc-table-wrap"><table class="doc-table"><tbody>${(b.rows||[]).map(r=>`<tr>${r.map(c=>`<td>${linkify(c,currentArticle)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;if(b.type==='image'&&/^images\/rules\/regles-[a-z0-9-]+\.svg$/.test(b.src||''))return `<figure class="rule-diagram"><picture>${/^images\/rules\/regles-[a-z0-9-]+-mobile\.svg$/.test(b.mobileSrc||'')?`<source media="(max-width: 700px)" srcset="${esc(b.mobileSrc)}">`: ''}<img src="${esc(b.src)}" alt="${esc(b.alt||'')}" loading="lazy"></picture><figcaption>${esc(b.caption||'')}</figcaption></figure>`;if(b.type==='image'&&/^images\/lore\/[a-z0-9-]+\.webp$/.test(b.src||''))return `<figure class="${b.style==='lore-reference'?'lore-reference':'lore-illustration'}"><img src="${esc(b.src)}" alt="${esc(b.alt||'')}" loading="lazy"><figcaption>${esc(b.caption||'')}</figcaption></figure>`;if(b.type==='image'&&/^images\/manual\/[a-z0-9-]+\.webp$/.test(b.src||''))return `<figure class="augmentation-variant-image"><img src="${esc(b.src)}" alt="${esc(b.alt||'')}" loading="lazy"><figcaption>${esc(b.caption||'')}</figcaption></figure>`;const style=(b.style||'').toLowerCase();const cls=style.includes('list')?' list':style.includes('spec')?' spec':style.includes('callout')?' callout':style.includes('lore')?' lore':'';return `<p class="body-p${cls}">${linkify(b.text||'',currentArticle)}</p>`}
