...
function metaBadges(a){return `<div class="meta"><span class="badge ${statusClass(a.status)}">${esc(statusLabel(a.status))}</span>${a.__editorialOverride?'<span class="badge canon">Édité</span>':''}${a.audience==='mj'?'<span class="badge mj">Contenu MJ</span>':''}${a.pnj?.completeness?`<span class="badge pnj-state ${esc(a.pnj.completeness)}">${esc(pnjCompletenessLabel(a.pnj.completeness))}</span>`:''}${(a.tags||[]).filter(Boolean).slice(0,8).map(t=>`<a class="badge tag-link" href="#/search?q=${encodeURIComponent(String(t))}" title="Rechercher toutes les entrées avec ce tag">${esc(t)}</a>`).join('')}</div>`}
...
async function router(){const raw=location.hash.slice(1)||'/start';if(raw==='/start')return showStart();if(raw.startsWith('/article/')){let x=decodeURIComponent(raw.slice(9)),anchor='';if(x.includes('@')){const parts=x.split('@',2);x=parts[0];anchor=decodeURIComponent(parts[1]||'')}if(!articleCache.has(x))await ensureCorpus();return showArticle(x,anchor)}await ensureCorpus();if(raw.startsWith('/category/'))return showCategory(decodeURIComponent(raw.slice(10)));if(raw.startsWith('/search'))return showSearch(new URLSearchParams(raw.split('?')[1]||'').get('q')||'');return showHome()}
