...
async function loadDataset(spec){
  ...
  const p=(async()=>{
    const texts=await Promise.all(Array.from({length:spec.parts},async(_,i)=>{const file=`${spec.prefix}-${String(i).padStart(2,'0')}.b64part`;const r=await fetch(`data/${file}`,{cache:'force-cache'});if(!r.ok)throw new Error(`${file} · HTTP ${r.status}`);return r.text()}));
...
function metaBadges(a){return `<div class="meta"><span class="badge ${statusClass(a.status)}">${esc(statusLabel(a.status))}</span>${a.audience==='mj'?'<span class="badge mj">Contenu MJ</span>':''}${a.pnj?.completeness?`<span class="badge pnj-state ${esc(a.pnj.completeness)}">${esc(pnjCompletenessLabel(a.pnj.completeness))}</span>`:''}${(a.tags||[]).filter(Boolean).slice(0,8).map(t=>`<span class="badge">${esc(t)}</span>`).join('')}</div>`}
...
function pnjFactsHtml(a){
  ...
  const portrait=p.portrait?`<div class="pnj-portrait"><img src="${esc(p.portrait)}" alt="Portrait de ${esc(a.title)}" loading="lazy"></div>`:'';
