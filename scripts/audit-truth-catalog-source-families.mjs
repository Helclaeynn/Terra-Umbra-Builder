...
const selected=source.entries.filter(entry=>['23','24','26'].includes(String(entry.chapter))||/(?:\bshi\b|jade|tao|ancrage)/i.test(norm(textOf(entry))));

console.log(`AUDIT VÉRITÉ — ${source.entries.length} entrées · chapitres 23/24/26 + ancrages associés ${selected.length}`);
