...
for(const old of legacy){
  ...
  for(const modern of current){
    ...
    if(oldTitle&&oldTitle===modernTitle){exact.push({legacyId:old.id,legacyTitle:old.title,modernId:modern.id,modernTitle:modern.title});continue}
    ...
    if(contentScore>=0.12)near.push({legacyId:old.id,legacyTitle:old.title,modernId:modern.id,modernTitle:modern.title,titleScore:+titleScore.toFixed(2),contentScore:+contentScore.toFixed(2)});
  }
}
...
}).map(page=>({id:page.id,title:page.title,sections:(page.sections||[]).length}));
...
for(const item of exact) console.log(`  EXACT | ${item.legacyId} | ${item.legacyTitle} -> ${item.modernId} | ${item.modernTitle}`);
console.log(`NEAR_DUPLICATES ${near.length}`);
for(const item of near.sort((a,b)=>b.contentScore-a.contentScore||b.titleScore-a.titleScore).slice(0,40)) console.log(`  NEAR | t=${item.titleScore} c=${item.contentScore} | ${item.legacyId} | ${item.legacyTitle} -> ${item.modernId} | ${item.modernTitle}`);
console.log(`SUSPICIOUS_LEGACY ${suspicious.length}`);
for(const item of suspicious) console.log(`  SUSPICIOUS | sections=${item.sections} | ${item.id} | ${item.title}`);
