...
function locateCards(decoded){
  ...
  for(const key of ['cards','entries','items','mechanics','techCards','tech_cards']) if(Array.isArray(decoded[key])) return {cards:decoded[key],path:`$.${key}`};
...
function bestSemantic(row,builder){
  const candidates=categoryCandidates(row.chapter,builder);
  const rowTitle=terms(row.title),rowText=terms(row.text);
  let best=null;
  for(const item of candidates){
    const titleScore=jaccard(rowTitle,terms(item.name));
    const effectScore=jaccard(rowText,terms([item.effect,item.description,item.access,item.prerequisiteName].filter(Boolean).join(' ')));
    const combined=Math.max(effectScore,titleScore*.85,(effectScore*.8)+(titleScore*.2));
    if(!best||combined>best.score)best={item,score:combined,titleScore,effectScore};
  }
  return best;
}
...
let b64='';
for(const part of manifest.parts){const file=`${SOURCE_DIR}/${part}`;if(!fs.existsSync(file))throw new Error(`Fragment V6 absent: ${part}`);b64+=fs.readFileSync(file,'utf8').replace(/\s+/g,'')}
...
if(!Array.isArray(rawCards)||rawCards.length!==882||rawCards.length!==manifest.entries)throw new Error(`Cartes V6: ${rawCards?.length||0}, attendu 882`);
...
const rows=cards.map((card,index)=>{const title=clean(card.title),chapter=clean(card.chapter),h3=clean(card.heading3),h4=clean(card.heading4),h5=clean(card.heading5),text=clean(card.text),matches=builderByName.get(normalize(title))||[];return{index:index+1,title,chapter,h3,h4,h5,text,matches,exact:matches.length>0}});
...
for(const row of rows){
  ...
  const pathKey=[row.chapter,row.h3||'—',row.h4||'—',row.h5||'—'].join(' › ');if(!headingStats.has(pathKey))headingStats.set(pathKey,{total:0,exact:0,unmatched:0});const hs=headingStats.get(pathKey);hs.total++;if(row.exact)hs.exact++;else hs.unmatched++;
}
const exact=rows.filter(row=>row.exact),unmatched=rows.filter(row=>!row.exact),exactNames=new Set(exact.map(row=>normalize(row.title)));
const v6NameCounts=new Map();for(const row of rows)inc(v6NameCounts,normalize(row.title));const duplicateV6Names=[...v6NameCounts.entries()].filter(([,count])=>count>1).sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0],'fr'));
const semantic=unmatched.filter(row=>CHAPTER_SCOPE[row.chapter]).map(row=>({...row,best:bestSemantic(row,builder)}));
...
console.log('SEMANTIC OVERLAP CANDIDATES');
for(const chapter of Object.keys(CHAPTER_SCOPE)){
  const scoped=semantic.filter(row=>row.chapter===chapter&&row.best);
  const c70=scoped.filter(row=>row.best.score>=.70).length,c50=scoped.filter(row=>row.best.score>=.50).length,c35=scoped.filter(row=>row.best.score>=.35).length;
  console.log(`  ch.${chapter}: >=0.70 ${c70} · >=0.50 ${c50} · >=0.35 ${c35} / ${scoped.length} non-exacts`);
  for(const row of scoped.sort((a,b)=>b.best.score-a.best.score).slice(0,12))console.log(`    ${(row.best.score).toFixed(3)} | ${row.title} ~= ${row.best.item.name} | effet ${(row.best.effectScore).toFixed(3)} · titre ${(row.best.titleScore).toFixed(3)}`);
}
...
console.log('UNMATCHED SAMPLE BY CHAPTER');
for(const chapter of [...new Set(unmatched.map(row=>row.chapter))].sort((a,b)=>Number(a)-Number(b))){
  console.log(`  CHAPTER ${chapter}`);
  for(const row of unmatched.filter(item=>item.chapter===chapter).slice(0,15)){const best=CHAPTER_SCOPE[chapter]?bestSemantic(row,builder):null;console.log(`    ${row.index} | ${row.h3||'—'} | ${row.h4||'—'} | ${row.h5||'—'} | ${row.title}${best?` | best ${(best.score).toFixed(3)} ${best.item.name}`:''}`)}
}
