...
const SOURCE_DIR='compendium/source/truth-lore-mages-angelus-v1';
...
function loadSources(){
  ...
  if(files.length!==2)throw new Error(`2 sources Mages/Angelus attendues, ${files.length}`);
  const pages=[];
  for(const file of files){
    ...
    if(!wrapper.page.title||!Array.isArray(wrapper.page.sections)||!wrapper.page.sections.length)throw new Error(`${file}: page source incomplète`);
    pages.push(wrapper.page);
  }
  for(const title of ['13. Mages','15. Angelus'])if(!pages.some(page=>page.title===title))throw new Error(`Source absente: ${title}`);
  return pages;
}
function navFor(title){
  if(title==='13. Mages')return {group:'Natures, peuples & traditions',groupOrder:20,subgroup:'Mages',subgroupOrder:30,pageOrder:10};
  if(title==='15. Angelus')return {group:'Natures, peuples & traditions',groupOrder:20,subgroup:'Angelus',subgroupOrder:50,pageOrder:10};
...
for(const item of sources){
  ...
  const canonical={
    ...
    status:'canon_recent',
    tags:['Vérité',item.title==='13. Mages'?'Mages':'Angelus','Lore V6','Lore book-first'],
    nav:navFor(item.title),
    loreEvidence:{chapter:item.title,paragraphs:item.evidence||[]},
    loreBook:{source:'Vérité V6',batch:'mages-angelus-v1'},
...
if(truth.length!==initialTruthCount)throw new Error(`La passe Mages/Angelus ne doit pas changer le nombre de pages Vérité (${truth.length}/${initialTruthCount})`);
if(legacy.length!==initialLegacyCount)throw new Error(`Le lore legacy doit rester inchangé (${legacy.length}/${initialLegacyCount})`);
const touched=truth.filter(page=>page.loreBook?.batch==='mages-angelus-v1');
if(touched.length!==2)throw new Error(`2 hubs Mages/Angelus attendus, ${touched.length}`);
...
for(const page of touched){
  ...
  if((page.sections||[]).length<8)throw new Error(`${page.title}: hub trop mince (${page.sections?.length||0} sections)`);
}
const ids=new Set();for(const page of truth){if(ids.has(page.id))throw new Error(`ID Vérité dupliqué: ${page.id}`);ids.add(page.id)}

const written=writeDataset('verite',truth,'v3-verite-lore-v5');
removePrefix('v3-verite-lore-v4');
manifest.expectedTotal=manifest.datasets.reduce((sum,d)=>sum+Number(d.count||0),0);
fs.writeFileSync(manifestPath,`${JSON.stringify(manifest,null,2)}\n`,'utf8');
console.log(`LORE MAGES/ANGELUS V6 — 2 hubs book-first remplacés · IDs conservés: ${replaced.map(x=>x.id).join(', ')}.`);
