...
function downloadJson(payload){
  downloadBlob(new Blob([JSON.stringify(payload,null,2)+'\n'],{type:'application/json'}),`tuc-compendium-overrides-${new Date().toISOString().slice(0,10)}.json`);
}
...
async function loadCommittedPackage(){
  ...
  if(payload?.version!==VERSION||!Array.isArray(payload.entries))throw new Error('manual-overrides.json commité invalide.');
  return payload;
}
async function exportPublicationBundle(){
  const items=await inspectDrafts();
  if(!items.length)throw new Error('Aucun brouillon à publier.');
  const blockers=items.filter(item=>['missing','invalid','conflict','media-missing','media-external'].includes(item.state));
  if(blockers.length)throw new Error(`Lot bloqué : ${blockers.map(item=>`${item.title} — ${item.label}`).join(' ; ')}`);

  const committed=await loadCommittedPackage();
  const payload=mergePublicationEntries(committed,items.map(item=>item.draft));
  const files=[{
    name:'compendium/data/manual-overrides.json',
    data:JSON.stringify(payload,null,2)+'\n',
  }];
  const addedMedia=new Set();
  for(const item of items){
    if(item.media?.state!=='media-local'||!item.media.item?.blob)continue;
    const repoPath=`compendium/${String(item.media.path).replace(/^compendium\//,'')}`;
    if(addedMedia.has(repoPath))continue;
    addedMedia.add(repoPath);
    files.push({name:repoPath,data:item.media.item.blob});
  }
  const instructions=[
    'TUC COMPENDIUM — LOT DE PUBLICATION',
    '',
    'Ce ZIP a été généré par le mode Édition.',
    'Copier son contenu à la racine du dépôt Terra-Umbra-Builder en conservant exactement les chemins.',
    '',
    `Overrides finaux : ${payload.entries.length}`,
    `Brouillons intégrés : ${items.length}`,
    `Médias WebP inclus : ${addedMedia.size}`,
    '',
    'Le fichier compendium/data/manual-overrides.json contient aussi les overrides déjà commités : ils ne sont pas perdus.',
    'Après copie/commit, laisser passer les gates Compendium V3 et Builder syntax check avant toute fusion vers main.',
  ].join('\n');
  files.push({name:'TUC-COMPENDIUM-PUBLICATION.txt',data:instructions+'\n'});
  const zip=await buildZip(files);
  downloadBlob(zip,`tuc-compendium-publication-${new Date().toISOString().slice(0,10)}.zip`);
  return {files:files.length,media:addedMedia.size,drafts:items.length,overrides:payload.entries.length};
}
...
function buildManager(){
  ...
  dialog.innerHTML=`<div class="editor-form"><header><div><div class="eyebrow">MODE ÉDITION</div><h2>Brouillons locaux</h2></div><button type="button" class="editor-close" data-action="close">×</button></header><div class="editor-draft-intro">Les brouillons sont contrôlés contre le corpus courant et leurs médias. Le <strong>lot de publication</strong> fusionne les corrections déjà commités, les nouveaux brouillons et les WebP locaux nécessaires.</div><div class="editor-draft-actions"><button type="button" class="editor-primary" data-action="publication">Exporter le lot de publication (.zip)</button><button type="button" data-action="export">Sauvegarde JSON des brouillons</button><button type="button" data-action="import">Importer / fusionner</button><input type="file" accept="application/json,.json" data-import-file hidden><span class="editor-spacer"></span><button type="button" class="editor-danger" data-action="delete-all">Tout supprimer</button></div><div class="editor-draft-list"></div><footer><span class="editor-spacer"></span><button type="button" data-action="close">Fermer</button></footer></div>`;
...
async function openDraftManager(){
  ...
  dialog.addEventListener('click',async event=>{
    ...
    if(action==='publication'){
      const button=event.target.closest('button');const original=button.textContent;button.disabled=true;button.textContent='Préparation du lot…';
      try{const result=await exportPublicationBundle();button.textContent=`Lot prêt ✓ · ${result.files} fichier(s)`;setTimeout(()=>button.textContent=original,1800);}catch(error){alert(error.message);button.textContent=original;}finally{button.disabled=false;}
      return;
    }
    if(action==='export'){
